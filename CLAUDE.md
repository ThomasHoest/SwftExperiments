...
### Architecture
- **`MdnsDiscovery.swift`** — `NetServiceBrowser` browsing `_bangolufsen._tcp.`, resolves to IPv4.
- **`MozartClient.swift`** — `URLSession` HTTP to `http://<host>/api/v1/...`.
- **`MozartEvents.swift`** — `URLSessionWebSocketTask`, exponential-backoff reconnect.
- **`Speaker.swift`** — `@Observable @MainActor`, parallel REST init, WS event handler. Throws `URLError` if `/beolink/self` is unreachable (so discovery can discard dead IPs).
- **`ContentView.swift`** — single screen: orb (breathing Task loop → mic-level driven), italic transcript, speaker list.
- **`SpeakerCardView.swift`** — card UI + `BeoColor` palette + `Color(hex:)` extension.
- **`VoiceToText.swift`** / **`AVService.swift`** — mic permission flow, `SFSpeechRecognizer`, RMS audio level callback for orb.
...
- Key REST endpoints: `/beolink/self`, `/playback/state`, `/sound/volume`, `/battery`, `/playback/sources/active`
