...
| 2026-05-04 | Implementation drift round-up | Reflects six implementation findings/changes that emerged during E-27..E-32 build-out and were not in the original epic text. Captured in `VoxioSpecification-1.2.md` "Decisions resolved during implementation" table. Summary: **(1)** ASE multi-room join switched from listener-side `OneWayJoin` to master-side `BuildExpandExperienceRequest` (`POST /BeoZone/Zone/ActiveSources/primaryExperience` with listener JID). Direction reversed: `target.client.join(peer: source.identifier)`. **(2)** ASE long-poll switched from `URLSession.data(for:)` to `URLSession.bytes(for:).lines` for real-time event delivery; `?timeout=55` added to URL. **(3)** ASE `/BeoZone/Zone/ActiveSources` REST shape corrected — `primaryExperience` and `activeSources` are sibling top-level keys, no `state` field; play state inferred from `source.inUse`. `api-spec-beonetremote.md` updated. **(4)** New ADR-002 introduces `NowPlayingPresentation` value type, `NowPlayingPresenter.make` pure function, `SpeakerSource` unified value type. `SpeakerClient.getActiveSource()` added to protocol requirement list (existential-dispatch fix). Replaces earlier `getActiveSourceName()`. **(5)** Widget storage refactored to per-host key namespace (`widget_speaker_<host>_…`) with `widget_known_hosts` index and play-state-aware auto-pick. `widget_data_version` 1 → 3. Intents gain `targetHost: String?` `@Parameter` so widget buttons act on the displayed speaker, not `SpeakerStore.activeSpeaker`. Widget bundle reads pre-rendered presentation strings; presenter runs on main-app side only. **(6)** `SpeakerDiscoveryService.didSettle` (2-second quiet-window debounce) added; HomeView startup auto-pick prefers a playing speaker over the first member. Plus minor: `mozartSetVolume` body key `volumeLevel` → `level` (was 400ing); `stop`/`pause` `isPlaying` guards removed; portrait-only orientation lock; `mapPlaybackState` accepts wider vocabulary; ASE PROGRESS_INFORMATION logging dropped to verbose. |
